...
function ResultsDisplay() {
  ...
  useEffect(() => {
    if (!quizId) return;
    try {
        const storedResult = localStorage.getItem(`mockTestResult-${quizId}`);
        if (storedResult) {
  ...
  return (
    <div className="container py-4 md:py-8 max-w-4xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl md:text-3xl font-headline">Mock Test Results: {course.name}</CardTitle>
        ...
        <CardContent className="space-y-6 md:space-y-8">
          ...
          <div className="text-center pt-4">
            <Button size="lg" onClick={() => router.push(`/mock-tests/${course.id}/start`)}>
